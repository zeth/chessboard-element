...
;(function () {
  ...
  class ChessBoardElement extends HTMLElement {
    ...
    constructor () {
      ...
      const buildSparePiecesHTML = (color) => {
        ...
        for (var i = 0; i < pieces.length; i++) {
          html += this.buildPieceHTML(pieces[i], false, sparePiecesElsIds[pieces[i]])
        }
      ...
      const beginDraggingPiece = (source, piece, x, y) => {
        ...
        $draggedPiece.attr('src', this.buildPieceImgSrc(piece)).css({
          display: '',
          position: 'absolute',
    ...
    animateSparePieceToSquare(piece, dest, completeFn) {
      var srcOffset = $('#' + sparePiecesElsIds[piece], this.shadowRoot).offset()
      ...
      var $animatedPiece = $('#' + pieceId, this.shadowRoot);
      $animatedPiece.css({
        display: '',
        position: 'absolute',
        left: srcOffset.left,
