...
;(function () {
  ...
  class ChessBoardElement extends HTMLElement {
    ...
    constructor () {
      ...
      const $container = this.$container = $(this.shadowRoot.querySelector('#container'));
      ...
      this.$sparePiecesTop = null;
      this.$sparePiecesBottom = null;
      ...
      const captureSquareOffsets = () => {
        ...
        for (var i in this.squareElsIds) {
          if (!this.squareElsIds.hasOwnProperty(i)) continue
      ...
      const snapbackDraggedPiece = () => {
        ...
        const square = this.shadowRoot.querySelector(`#${this.squareElsIds[draggedPieceSource]}`);
        const rect = square.getBoundingClientRect();
      ...
      const trashDraggedPiece = () => {
        ...
        this.$draggedPiece.fadeOut(config.trashSpeed)
      ...
      const updateDraggedPiece = (x, y) => {
        ...
        const draggedPieceElement = this.$draggedPiece.get()[0];
        ...
        if (validSquare(draggedPieceLocation)) {
          const previousSquare = this.shadowRoot.querySelector('#' + this.squareElsIds[draggedPieceLocation]);
      ...
      const initDOM = () => {
        ...
        if (config.sparePieces) {
          this.$sparePiecesTop = $container.find('.' + CSS.sparePiecesTop)
          this.$sparePiecesBottom = $container.find('.' + CSS.sparePiecesBottom)
        }
    ...
    doAnimations(animations, oldPos, newPos) {
      ...
      for (const animation of animations) {
        ...
        // add a piece with spare pieces - animate from the spares
          ...
          console.log('add animation from spare pieces');
    ...
    animateSparePieceToSquare(piece, dest, completeFn) {
      var srcOffset = $('#' + this.sparePiecesElsIds[piece], this.shadowRoot).offset()
      var $destSquare = $('#' + this.squareElsIds[dest], this.shadowRoot)
    ...
    animateSquareToSquare(src, dest, piece, completeFn) {
      ...
      var $srcSquare = $('#' + this.squareElsIds[src], this.shadowRoot);
